...
#  DQN Training Loop (Gym-compatible)
    ...
    for ep in range(episodes):
        ...
        if (ep + 1) % 50 == 0:
            ...
            print(f"Episode {ep+1}/{episodes} | "
                  f"Avg Reward: {avg_r:.2f} | Avg Loss: {avg_l:.4f} | "
                  f"Avg Q: {avg_q:.4f} | Epsilon: {epsilon:.2f}")

    return q_net, rewards_log, losses_log, avg_q_log

#  Q-Table Extraction for Animation
...
# Train agent
q_net, rewards_log, losses_log, avg_q_log = train_dqn(env, episodes=400)
