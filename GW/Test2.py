...
for size in grid_sizes:
    print(f"\n--- Training for grid size {size} (MOVING OBSTACLES) ---")

    # Create environment with moving obstacles and static goal
    moving_env = GridWorldEnv(
        size=size,
        goals=[{'pos_init': (size - 2, size - 1), 'moving': False}],  # Goal is static
        obstacles=[
            {'pos_init': (1, 1), 'moving': True},
            {'pos_init': (2, 2), 'moving': True},
            {'pos_init': (1, 3), 'moving': True}
        ] if size > 3 else []  # Skip obstacles in small grids
    )

    # Create trainer with the dynamic environment
...
plt.title('Episodes vs Average Return (Moving Obstacles)')
plt.xlabel('Episode')
...
plt.title('Episodes vs Steps per Episode (Moving Obstacles)')
plt.xlabel('Episode')
plt.ylabel('Steps')
plt.legend()
plt.grid()
...
plt.title('Final Performance vs Grid Size (Moving Obstacles)')
